...
@Component({
    ...
    Enter User EmailId : <input type="text" #emailId> 
     <button type="submit" (click) = "sold(userName.value, emailId.value)">Confirm</button>
                 ...
                 <table>
                 <tr>
                 <td><input type="search" class="search_1" placeholder="Enter Location"  #location/>
                 <td><button type="submit" (click) = "searchByLocation(location.value)" >Search</button>        
                </tr>
                </table>
                 </form>
  ...
  export class SearchByLocationComponent{
    ...
    searchByLocation(location){
        ...
        this.service.searchByLocation(location).subscribe( data => {this.properties = data;})
