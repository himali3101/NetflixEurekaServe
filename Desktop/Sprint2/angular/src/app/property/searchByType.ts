...
@Component({
    ...
    Enter User EmailId : <input type="text" #emailId> 
     <button type="submit" (click) = "sold(userName.value, emailId.value)">Confirm</button>
                 ...
                 <table>
                 <tr>
                 <td><input type="search" class="search_1" placeholder="Enter type"  #propertyType/>
                 <td><button type="submit" (click) = "searchByType(propertyType.value)" >Search</button>        
                </tr>
                </table>
                 </form>
  ...
  export class SearchByTypeComponent{
    ...
    id : number;
    constructor(private router: Router, private service : RealestateService, private propertySold : PropertySoldComponent){ }
